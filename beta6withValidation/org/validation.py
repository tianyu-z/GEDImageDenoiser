...
parser.add_argument("--beta", type=float, default=6, help='beta of GED')
...
def main():
    ...
    for epoch in range(opt.epochs):
        ...
        PATH = os.path.join(opt.outf, model_file)
        ...
        for k in range(len(dataset_val)):
            ...
            alpha = getAlpha(opt.val_noiseL/255.,opt.beta)
            flatSize = getSize(img_val)
            noise = torch.FloatTensor(gennorm.rvs(opt.beta, scale = alpha, size = flatSize, random_state = None))
            noise = noise.view(img_val.size())
        ...
        model_file = 'modelval_' + str(epoch) + '.pth'
        torch.save(model.state_dict(), os.path.join(opt.outf, model_file))
        PSNRs.append(psnr_val)
        modelBest_file = 'model_Best_val.pth'
        if epoch == 0:
